...
export class SchedulerImpl {
  ...
  addEF(func: () => unknown, params: unknown[] = []) {
  ...
  delay(func: () => unknown, params: unknown[], delay: number) {
  ...
  defer(func: () => unknown, params: unknown[] = []) {
    this._deferTasks.push({ func, params });
    this._loop();
  }

  next(func: () => unknown, params: unknown[] = []) {
    this._nextTasks.push({ func, params });
    this._loop();
  }

  usurp(func: () => unknown, params: unknown[] = []) {
...
export class Scheduler {
  private static instance = new SchedulerImpl();
